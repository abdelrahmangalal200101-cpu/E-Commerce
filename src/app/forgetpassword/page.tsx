...
import { z } from "zod";
import { FaShieldAlt, FaClock, FaTruck, FaLock, FaStar, FaUsers } from "react-icons/fa";
...
export default function ForgotPasswordPage() {
  ...
  async function onSubmit(values: ForgotSchemaType) {
    ...
    if (data.statusMsg === "success") {
      toast.success("Reset code sent to your email!", { duration: 3000, position: "top-center" });
      setTimeout(() => router.push("/verifypassword"), 1500);
    } else {
      toast.error(data.message || "Something went wrong", { duration: 3000, position: "top-center" });
    }
    setIsLoading(false);
  }
      ...
      <div className="flex w-full flex-col lg:flex-row items-center gap-8 lg:gap-12">
        ...
        <div className="hidden lg:flex flex-1 flex-col gap-6">
          <div className="rounded-[16px] overflow-hidden w-full max-w-154 h-96">
            <img src={cover.src} alt="Forgot Password" className="w-full h-full object-cover object-center" />
          </div>
          <div className="flex flex-col justify-center items-center text-center gap-4">
            ...
            <p className="text-base xl:text-lg text-[#4A5565] font-medium">
              Don't worry, it happens to the best of us. We'll help you get back <br className="hidden xl:block" /> into your account in no time.
            </p>
        ...
        >
          ...
          <h4 className="font-semibold text-2xl text-Links mt-1">Forgot Password?</h4>
          ...
          <div className="flex flex-col gap-5 sm:gap-7 mt-2">
            <Field data-invalid={!!errors.email}>
              <FieldLabel htmlFor="email" className="text-base font-medium text-Links text-left">
                Email Address
              </FieldLabel>
            ...
            <p className="text-base font-medium pt-6 sm:pt-10 border-t border-[#D1D5DC4D] text-Links">
              ...
              <Link className="text-homeGreen hover:text-green-700 cursor-pointer transition-all duration-200" href="/login">
                Sign In
              </Link>
            </p>
